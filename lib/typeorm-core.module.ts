  ...
  static forRoot(options: TypeOrmModuleOptions = {}): DynamicModule {
    ...
    const connectionProvider = {
      ...
      useFactory: async (): Promise<Connection> => {
        const conn = await this.createConnectionFactory(options);
        if (options.proxy) {
          return ProxyUtil.proxyConn(conn);
        }
        return conn;
      },
    };
  ...
  static forRootAsync(options: TypeOrmModuleAsyncOptions): DynamicModule {
    const connectionProvider = {
      ...
      useFactory: async (
        ...
        if (options.name) {
          const conn = await this.createConnectionFactory({
            ...typeOrmOptions,
            name: options.name,
          });
          if (typeOrmOptions.proxy) {
            return ProxyUtil.proxyConn(conn);
          }
          return conn;
        }

        const conn = await this.createConnectionFactory(typeOrmOptions);
        if (typeOrmOptions.proxy) {
          return ProxyUtil.proxyConn(conn);
        }
        return conn;
